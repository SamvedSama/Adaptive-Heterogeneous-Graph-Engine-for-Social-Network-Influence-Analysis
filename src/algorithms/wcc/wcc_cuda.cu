 ...
 *
 * Strategy: edge-partition model.
 *   GPU 0 processes edges [0,   E/2)
 *   GPU 1 processes edges [E/2, E)
 *   Both GPUs hold the full parent[] array (n nodes).
 *   After each round the host merges both parent arrays (elementwise min)
 *   and uploads the merged result back to both GPUs.
 *   Final flatten runs on GPU 0 only after convergence.
 *
 * Gplus OOM fix:
 *   Each GPU allocates only E/2 edges instead of E.
 *   Host COO is freed immediately after upload to reduce peak RSS.
 *
 * Falls back to single-GPU when only 1 GPU present or E < 200,000.
 */
...
// ---------------------------------------------------------------------------
// Helper: build WccResult from flattened parent array
// ---------------------------------------------------------------------------
namespace
{

    static WccResult build_result(const std::vector<unsigned> &parent,
                                  NodeID n, bool verbose)
    {
        WccResult out;
        ...
        if (verbose)
        {
            ...
            for (int t = 0; t < kShow; ++t)
                std::printf("%u:%d ",
                            static_cast<unsigned>(out.component_sizes[t].first),
...
WccResult wcc_cuda(const Graph &graph, bool verbose)
{
    ...
    // Host parent: identity initialisation
    ...
    if (!use_multi)
    {
        ...
        // Lambda defined HERE — in scope for the entire single-GPU block
        auto cleanup = [&]()
        {
            cudaFree(d_parent);
            d_parent = nullptr;
        ...
        // Build COO on host and upload, then immediately free host copy
        {
            std::vector<unsigned> buf_u(E_sz), buf_v(E_sz);
            for (NodeID v = 0; v < n; ++v)
            {
                ...
                {
                    buf_u[static_cast<std::size_t>(e)] = static_cast<unsigned>(v);
            ...
            // Free host COO immediately to reduce peak RSS
            buf_u.clear();
            buf_u.shrink_to_fit();
            buf_v.clear();
            buf_v.shrink_to_fit();
    ...
    // Build full host COO (needed to partition across GPUs)
    ...
    for (int g = 0; g < 2; ++g)
    {
        ...
        checkCuda(cudaMemcpy(d_parent[g], host_parent.data(),
                             static_cast<std::size_t>(n) * sizeof(unsigned), cudaMemcpyHostToDevice));
        checkCuda(cudaMemcpy(d_eu_g[g],
                             h_eu.data() + part_start_e[g], eb, cudaMemcpyHostToDevice));
        checkCuda(cudaMemcpy(d_ev_g[g],
                             h_ev.data() + part_start_e[g], eb, cudaMemcpyHostToDevice));
    }

    // Free host COO — no longer needed
    h_eu.clear();
    h_eu.shrink_to_fit();
    h_ev.clear();
    h_ev.shrink_to_fit();

    // Pinned parent readback buffers (one per GPU)
    ...
    for (int round = 0; round < max_rounds; ++round)
    {
        ...
        // Launch hook + 2× shortcut on both GPUs concurrently
        for (int g = 0; g < 2; ++g)
        {
            checkCuda(cudaSetDevice(g));
        ...
        // Sync both GPUs and read changed flags
        int hc[2] = {0, 0};
        for (int g = 0; g < 2; ++g)
        {
            checkCuda(cudaSetDevice(g));
        ...
        // Read both parent arrays to host
        for (int g = 0; g < 2; ++g)
        {
            checkCuda(cudaSetDevice(g));
        ...
        // Merge: elementwise min (correct by SV monotone invariant)
        ...
        // Broadcast merged parent back to both GPUs
        for (int g = 0; g < 2; ++g)
        {
            checkCuda(cudaSetDevice(g));

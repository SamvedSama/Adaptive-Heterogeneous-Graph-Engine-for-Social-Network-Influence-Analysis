...
static Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (streq(argv[i], "--graph") && i + 1 < argc) {
            ...
            std::fprintf(stderr,
                         "Usage: benchmark_runner --graph <path> [--dataset name] [--trials N]\n");
...
/**
 * @brief Picks a writable CSV location, preferring the source-tree path from the build directory.
 */
static std::string pick_results_csv_path() {
    namespace fs = std::filesystem;
    const char* candidates[] = {"benchmarks/results/results.csv", "../benchmarks/results/results.csv"};
    ...
    return std::string("benchmarks/results/results.csv");
}
...
static void append_csv_row(const std::string& csv_path,
    ...
    out << algorithm << ',' << mode << ',' << dataset << ',' << static_cast<unsigned long long>(num_nodes)
        << ',' << static_cast<unsigned long long>(num_edges) << ',' << time_ms << '\n';
}
...
static void print_median_speedup_summary(
    const std::map<Key, std::vector<double>>& buckets, const char* algo, bool gpu_present) {
...
int main(int argc, char** argv) {
    ...
    const std::string csv_path = pick_results_csv_path();
    bool header_exists = false;
    {
        std::ifstream probe(csv_path);
        if (probe) {
            std::string line;
            if (std::getline(probe, line) && line.rfind("algorithm", 0) == 0) {
                header_exists = true;
            }
        }
    }
    ...
    for (const auto& job : workloads) {
        ...
        std::string algo;
        std::string mode;
        if (key.rfind("bfs_", 0) == 0) {
            algo = "bfs";
            mode = key.substr(4);
        } else if (key.rfind("pagerank_", 0) == 0) {
            algo = "pagerank";
            mode = key.substr(9);
        } else if (key.rfind("wcc_", 0) == 0) {
            algo = "wcc";
            mode = key.substr(4);
        } else {
            algo = key;
        ...
        for (int t = 0; t < args.trials; ++t) {
            ...
            std::printf("[compute] %s trial %d/%d finished: %.3f ms\n", key.c_str(), t + 1, args.trials,
                        ms);
            append_csv_row(csv_path, &header_exists, algo, mode, args.dataset_name, g.num_nodes(),
                           g.num_edges(), ms);

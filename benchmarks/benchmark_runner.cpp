...
static Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (streq(argv[i], "--graph") && i + 1 < argc) {
            ...
            std::fprintf(stderr,
                         "Usage: benchmark_runner --graph <path> [--dataset name] [--trials N] [--outcsv <path>]\n");
...
int main(int argc, char** argv) {
    ...
    const std::string& csv_path = args.csv_path;
    if (csv_path.empty()) {
        std::fprintf(stderr, "benchmark_runner: --outcsv <path> is required\n");
        std::exit(EXIT_FAILURE);
    }
    {
        namespace fs = std::filesystem;
        std::error_code ec;
        fs::create_directories(fs::path(csv_path).parent_path(), ec);
    }
